...
export const Filter = ({ setFilteredProject, projectsNoFiltered,number }: FilterProps) => {
      ...
      <details className='dropdown'>
        ...
        <ul className='menu dropdown-content bg-base-100 rounded-box z-10 w-52 p-2 shadow-sm'>
          {technologies.map((technologie)=>{
            return <li key={technologie}><a onClick={()=> handleFilter(technologie)}>{technologie}</a></li>
          })}
        </ul>
      </details>
      <p>
