...
export const projectService = {
  ...
  // Ottieni linguaggi di un progetto
  getLanguages: async (projectPath: string) => {
    try {
      const response = await axios.get(
        API_ENDPOINTS.GITHUB.LANGUAGES(projectPath),
      );
      return response;
    } catch (error) {
      if (error instanceof AxiosError)
        throw new Error(
          error.response?.data?.message || "Errore nel recupero dei linguaggi",
        );
      throw error;
    }
  },
